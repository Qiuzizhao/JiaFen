...
- 奖励机会：顶栏「奖励」进入，可按班级查看全部小组，或汇总查看仍有待奖励机会的小组；每达到一个可设置的积分档位，首次发放一个机会。旧数据首次启用时按当前积分初始化已达档位。扣分、清零或调低分数后再涨分都不会重复发放；点击机会图标即可在待奖励 / 已完成之间切换，多设备实时同步
- 奖励档位按账号设置。更改档位后，从各组历史最高分之后开始计算；已发机会和完成状态会随学期备份导出、导入
...
- **顶部操作栏**：只显示「班级 / 奖励 / 语音播报 / 投屏模式 / 账号」，隐藏「导出备份 / 导入备份」（数据备份请在桌面端操作）；进入奖励页后顶栏按钮变为「退出奖励」。
